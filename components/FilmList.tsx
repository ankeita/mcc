...
const FilmList=(props : any)=>{
	...
	//functons locales
	  ...
	  const _isFavoriteFilm=(idFilm: number)=>{
		...
		const foundFilm = props.favoritesFilm.find((f: any) => f.id === idFilm);
	...
	return (
		<View style={styles.main_container}>		 
		  <FlatList
			...
			renderItem={({ item }) => <FilmItem film={item} displayDetailForFilm={_displayDetailForFilm} isFavoriteFilm={_isFavoriteFilm(item.id)}/>}
			...
			extraData={props.favoritesFilm} //LE flatList se re render que si "data" et "extradata" ont changé
		  />		  
		</View>
	  )
}
